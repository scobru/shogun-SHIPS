...
try {
  ...
  console.warn(
    "⚠️  Fluidkey/scure not available. Install with: yarn add @fluidkey/stealth-account-kit @scure/bip32"
  );
}
...
class SHIP_03 implements ISHIP_03 {
  ...
  async getPublicStealthKeysByUsername(username: string): Promise<{
    ...
    try {
      ...
      console.log(
        `✅ Resolved ${username} → ${userData.userPub.slice(0, 20)}...`
      );
  ...
  async getPublicStealthKeys(userPub: string): Promise<{
    ...
    try {
      ...
      console.log(
        `🔍 Loading stealth keys for pub: ${userPub.slice(0, 20)}...`
      );
      ...
      if (!data || !data.viewingKey || !data.spendingKey) {
        console.log(
          `❌ No stealth keys found for user: ${userPub.slice(0, 20)}...`
        );
   ...
   */
  async searchStealthDirectory(): Promise<
    Array<{
      username?: string;
      gunPub: string;
      viewingPublicKey: string;
  ...
  > {
    try {
      ...
      console.log(
        "💡 Users can share their username or Gun pub for stealth payments"
      );
  ...
  async generateEphemeralKeyPair(): Promise<EphemeralKeyPair> {
    ...
    if (this.viewingKey) {
      try {
        ...
        console.warn(
          "Fluidkey ephemeral key generation failed, using fallback:",
          error
        );
      }
    }
  ...
  async generateStealthAddress(
    ...
    try {
      ...
      console.log(
        "🔑 Using ephemeral key (first 10 chars):",
        ephemeralKey.slice(0, 10) + "..."
      );
  ...
  async openStealthAddress(
    ...
    try {
      ...
      console.log(
        "  Viewing private:",
        this.viewingKey.privateKey.slice(0, 10) + "..."
      );
      console.log(
        "  Spending private:",
        this.spendingKey.privateKey.slice(0, 10) + "..."
      );
      console.log(
        "  Ephemeral public:",
        normalizedEphemeralKey.slice(0, 20) + "..."
      );
      ...
      if (generateStealthPrivateKey) {
        try {
          ...
          console.warn(
            "⚠️  Fluidkey failed, using fallback method:",
            fluidkeyError
          );
  ...
  private async deriveStealthKeysFromIdentity(): Promise<void> {
    try {
      ...
      console.log(
        "📍 Keys are compatible with Fluidkey stealth address operations"
      );
        ...
        gun
          ...
          .put(publicKeys, (ack: any) => {
            if (ack.err) {
              ...
              console.log(
                `📍 Published at: ${userPub.slice(0, 20)}/${SHIP_03.NODES.STEALTH_KEYS_PUBLIC}`
              );
              resolve();
            }
          });
