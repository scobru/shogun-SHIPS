...
class SHIP_05 implements ISHIP_05 {
  ...
  private async uploadToIPFS(data: File | Buffer): Promise<string | null> {
    try {
      ...
      if (
        this.ipfsStorage &&
        typeof this.ipfsStorage.uploadBuffer === "function"
      ) {
        try {
      ...
      if (service === "PINATA" && this.config.ipfsConfig?.pinataJwt) {
        ...
        if (isNode) {
          ...
          try {
            ...
            throw new Error(
              `Invalid IPFS response: ${responseText.substring(0, 100)}`
            );
          }
        ...
        if (isNode) {
          ...
          const filename =
            data instanceof Buffer ? "encrypted-file.bin" : "file.bin";

          console.log(`📦 Preparing upload:`);
          console.log(
            `   Data type: ${data instanceof Buffer ? "Buffer" : typeof data}`
          );
          console.log(
            `   Data length: ${data instanceof Buffer ? data.length : (data as File).size || "unknown"}`
          );
          ...
          const tryUpload = (endpoint: string): Promise<string> => {
            return new Promise((resolve, reject) => {
              ...
              const request = httpModule.request(options, (response: any) => {
                ...
                response.on("end", () => {
                  console.log(
                    `📡 ${endpoint} response: ${response.statusCode}`
                  );
                  ...
                  if (response.statusCode === 200) {
                    try {
                      ...
                      if (hash) {
                        ...
                        reject(
                          new Error(`No hash in response from ${endpoint}`)
                        );
                      }
                    } catch (parseError) {
                      reject(
                        new Error(
                          `Invalid JSON from ${endpoint}: ${responseData.substring(0, 100)}`
                        )
                      );
                    }
                  } else {
                    reject(
                      new Error(
                        `${endpoint} failed (${response.statusCode}): ${responseData.substring(0, 100)}`
                      )
                    );
                  }
                });
          ...
          for (const endpoint of endpoints) {
            try {
              ...
              console.log(
                `✅ Upload successful via ${endpoint}! Hash: ${hash}`
              );
              return hash;
            } catch (error: any) {
          ...
          throw new Error(
            "All upload endpoints failed. Check API URL and token."
          );
        } else {
          // Browser environment
          let apiUrl =
            this.config.ipfsConfig?.customApiUrl || "https://ipfs.io";
            ...
            const request = httpModule.request(options, (response: any) => {
              ...
              response.on("end", () => {
                if (response.statusCode === 200) {
                  ...
                  console.warn(
                    `⚠️  Unpin failed (${response.statusCode}): ${data.substring(0, 100)}`
                  );
                  resolve(false);
                }
              });
    ...
    try {
      ...
      if (this.ipfsStorage && typeof this.ipfsStorage.get === "function") {
        try {
          ...
          console.warn(
            "⚠️ shogun-ipfs download failed, using fallback:",
            error
          );
          // Continue to fallback
        }
              ...
              const result = await new Promise<string>((resolve, reject) => {
                ...
                const request = httpModule.request(options, (response: any) => {
                  ...
                  response.on("end", () => {
                    if (response.statusCode === 200) {
                      console.log(
                        `✅ Download successful via ${endpoint.path}`
                      );
                      resolve(data);
                    } else {
