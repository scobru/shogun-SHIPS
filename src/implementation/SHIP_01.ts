...
class SHIP_01 implements ISHIP_01 {
  ...
  constructor(identity: ISHIP_00) {
    if (!identity.isLoggedIn()) {
      throw new Error(
        "User must be authenticated via SHIP-00 before using SHIP-01"
      );
    }
  ...
  async sendMessage(
    ...
    try {
      ...
      const encryptedMessage = await crypto.encFor(message, senderPair, {
  ...
  async sendMessageWithToken(
    ...
    try {
      ...
      console.log(
        `✅ Token message sent: ${messageId} (channel: ${channel || "default"})`
      );
  ...
  async listenForTokenMessages(
    ...
    console.log(
      `👂 Listening for token messages (channel: ${channel || "all"})...`
    );
  }
}
