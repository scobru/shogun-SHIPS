...
class SHIP_02 implements ISHIP_02 {
  ...
  async initialize(useMnemonic: boolean = false): Promise<void> {
    ...
    try {
      ...
      if (useMnemonic) {
        ...
        console.log(
          "✅ HD wallet initialized from mnemonic (MetaMask compatible)"
        );
      } else {
  ...
  async getPrimaryAddress(): Promise<string> {
    ...
    if (!this.hdWallet) {
      throw new Error(
        "HD wallet not initialized. Please call initialize() before deriving addresses."
      );
    }
  ...
  private async saveAddressBookToGun(addressBook: AddressBook): Promise<void> {
    try {
      ...
      await user
        .get(SHIP_02.NODES.ADDRESS_BOOK)
        .put(JSON.stringify(addressBook));
  ...
  async importGunPair(pairData: string): Promise<boolean> {
    try {
      ...
      console.log(
        "💡 Use identity.importKeyPair() instead for Gun key restoration"
      );
  ...
  async importAllUserData(
    ...
    try {
      ...
      if (options.importMnemonic && data.mnemonic) {
        try {
          const mnemonicDecrypted = await this.decryptSensitiveData(
            data.mnemonic
          );
  ...
  async initializeWalletPaths(): Promise<void> {
    try {
      ...
      if (count === 0) {
        console.log(
          "No wallet paths found, new wallets will be created when needed"
        );
      } else {
  ...
  private async saveWalletPathsToGun(): Promise<void> {
    try {
      ...
      await user
        .get(SHIP_02.NODES.WALLET_PATHS)
        .put(JSON.stringify(this.walletPaths));
