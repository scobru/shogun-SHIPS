...
class SHIP_06 implements ISHIP_06 {
  ...
  constructor(identity: ISHIP_00, vaultNodeName?: string) {
    if (!identity.isLoggedIn()) {
      throw new Error(
        "User must be authenticated via SHIP-00 before using SHIP-06"
      );
    }
            ...
            try {
              ...
              if (crypto && pair && record.metadata) {
                ...
                if (
                  ...
                  metadata.type !== options.filterByType
                ) {
                  return;
                }
                ...
                if (options.filterByTag) {
                  if (
                    !metadata.tags ||
                    !metadata.tags.includes(options.filterByTag)
                  ) {
                    return;
                  }
                }
  ...
  async import(
    ...
    try {
      ...
      if (importData.version !== SHIP_06.VAULT_VERSION) {
        console.warn(
          `⚠️  Version mismatch: ${importData.version} vs ${SHIP_06.VAULT_VERSION}`
        );
      }
      ...
      console.log(
        `✅ Vault imported: ${importedCount} records (${skippedCount} skipped)`
      );
  ...
  async compact(): Promise<VaultResult> {
    ...
    try {
      ...
      console.log(
        `✅ Vault compacted: ${compactedCount} records permanently removed`
      );
    ...
    try {
      ...
      for (const name of allRecords) {
        ...
        if (record) {
          ...
          if (
            dataString.includes(queryLower) ||
  ...
  private async updateRecordCount(): Promise<void> {
    try {
      ...
      await this.metadataNode
        .get("recordCount")
        .put(activeRecords.length.toString())
        .then();
    } catch (error) {
