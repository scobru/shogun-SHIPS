        ...
        if (command === "exit" || command === "quit") {
          ...
          if (args.length === 0) {
            ...
            const type = await prompt(
              "Type (password/apiKey/privateKey/note): "
            );
          ...
          if (args.length === 0) {
            ...
            if (record) {
              ...
              if (record.metadata) {
                ...
                console.log(
                  `      Description: ${record.metadata.description || "N/A"}`
                );
                console.log(
                  `      Tags: ${record.metadata.tags?.join(", ") || "N/A"}`
                );
              }
            } else {
          ...
          const confirm = await prompt(
            "Permanently remove deleted records? (y/n): "
          );
