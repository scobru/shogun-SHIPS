...
export class MessengerCLI {
    ...
    try {
      ...
      if (this.currentChannel && this.channelToken) {
        ...
        console.log(
          c(
            "yellow",
            "⚠️  Usa /to <username> per messaggi diretti o /channel <nome> <token> per canali"
          )
        );
        return;
      }
  ...
  private async importKeyPair(base64KeyPair: string): Promise<void> {
    try {
      ...
      if (
        ...
        console.log(
          c("red", "❌ Key pair invalido. Mancano campi obbligatori.")
        );
        return;
      }
      ...
      console.log(
        c("gray", `   Public Key: ${keyPairData.pub.substring(0, 30)}...`)
      );
      console.log("");
  ...
  private async loginWithPair(base64KeyPair: string): Promise<void> {
    ...
    try {
      ...
      if (result.success) {
        ...
        this.derivedAddress = await this.identity.deriveEthereumAddress(
          seaPair.pub
        );
        this.isAuthenticated = true;
        ...
        console.log(
          c("gray", `   Public Key: ${seaPair.pub.substring(0, 40)}...`)
        );
        console.log(
          c("gray", `   Derived Address: ${c("cyan", this.derivedAddress)}`)
        );
        console.log("");
  ...
  private async wipeAllMessages(): Promise<void> {
    ...
    console.log(
      c(
        "yellow",
        "⚠️  ATTENZIONE: Stai per cancellare TUTTI i messaggi dal nodo GunDB"
      )
    );
    ...
    // Conferma
    this.rl.question(
      c("red", "Sei sicuro? Scrivi 'CONFERMA' per procedere: "),
      (answer: string) => {
        if (answer.trim() === "CONFERMA") {
          ...
          gun.get(SHIP_01.NODES.MESSAGES).once((allMessages: any) => {
            if (allMessages && typeof allMessages === "object") {
              ...
              console.log(
                c("green", `✅ ${count} messaggi cancellati dal nodo GunDB`)
              );
            } else {
    ...
    switch (command) {
      ...
      case "/chat":
        ...
        if (args[0]) {
          ...
          const history = await this.messaging.getMessageHistory(
            this.recipient
          );
          if (history.length > 0) {
            console.log(
      ...
      case "/login-pair":
        if (args[0]) {
          ...
          console.log(
            c("yellow", "⚠️  Uso: /login-pair <keypair-json-base64>")
          );
        }
        break;
  ...
  private showHelp(): void {
    ...
    console.log(
      "  " + c("bold", "/login <user> <pass>") + "  - Login/Signup con password"
    );
    console.log(
      "  " + c("bold", "/login-pair <keypair>") + "  - Login con SEA key pair"
    );
    console.log("");
    console.log(c("bold", "MESSAGGISTICA:"));
    console.log(
      "  " + c("bold", "/to <username>") + "        - Chat diretta con utente"
    );
    console.log(
      "  " +
        c("bold", "/channel <nome> <token>") +
        " - Entra in canale criptato"
    );
    console.log(
      "  " + c("bold", "/leave") + "                - Esci dal canale"
    );
    console.log("");
    ...
    console.log(
      "  " +
        c("bold", "/export") +
        "                - Esporta il tuo key pair (backup)"
    );
    console.log(
      "  " + c("bold", "/import <keypair>") + "     - Mostra info su key pair"
    );
    console.log("");
    console.log(c("bold", "UTILITÀ:"));
    console.log(
      "  " + c("bold", "/help") + "                 - Mostra questo aiuto"
    );
    console.log(
      "  " + c("bold", "/status") + "               - Mostra stato connessione"
    );
    console.log(
      "  " + c("bold", "/clear") + "                - Pulisci schermo"
    );
    console.log(
      "  " +
        c("bold", "/wipe") +
        "                 - Cancella TUTTI i messaggi (⚠️)"
    );
  ...
  private showStatus(): void {
    ...
    if (this.currentChannel) {
      ...
      console.log(
        "  " + c("gray", "Chat con:") + " " + c("yellow", this.recipient)
      );
    } else {
      console.log(
        "  " + c("gray", "Destinatario:") + " " + c("gray", "(nessuno)")
      );
    }
