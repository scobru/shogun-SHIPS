...
class StorageCLI {
  ...
  async configureIPFS(): Promise<void> {
    ...
    if (this.loadIPFSConfig()) {
      ...
      const reconfig = await this.prompt(
        "Use saved config? (y/n, default: y): "
      );
  ...
  async configureCustomGateway(): Promise<void> {
    ...
    console.log(
      "   Shogun Relay (remote): https://relay.shogun-eco.xyz/api/v1/ipfs"
    );
    ...
    console.log(
      `✅ Custom gateway configured: ${this.ipfsConfig.customApiUrl}`
    );
  }
