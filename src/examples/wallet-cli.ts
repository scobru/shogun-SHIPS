...
class WalletCLI {
  ...
  private async authenticate(password: string): Promise<void> {
    ...
    try {
      ...
      if (loginResult.success) {
        ...
        console.log(
          `📍 Public Key: ${loginResult.userPub?.substring(0, 20)}...`
        );
      } else {
        // If login fails, try signup
        console.log("🔄 User not found, creating new identity...");
        const signupResult = await this.identity.signup(
          this.username,
          password
        );
  ...
  private async deriveBIP44Address(): Promise<void> {
    ...
    const changeStr = await this.prompt(
  ...
  private async deriveMultipleAddresses(): Promise<void> {
    ...
    const results = await this.wallet.deriveMultipleAddresses(
      count,
      startIndex
    );
  ...
  private async viewExportMnemonic(): Promise<void> {
    ...
    if (!mnemonic) {
      ...
      console.log(
        "💡 To use mnemonic mode, re-initialize with mnemonic option"
      );
      return;
    }
  ...
  private async verifyMetaMaskAddresses(): Promise<void> {
    ...
    const standardAddresses = this.wallet.getStandardBIP44Addresses(
      mnemonic,
      5
    );
  ...
  private async signTransaction(): Promise<void> {
    ...
    try {
      ...
      if (result.success) {
        ...
        console.log(
          `   Signed Transaction: ${result.signedTransaction?.substring(0, 50)}...`
        );
        console.log(
  ...
  private async exportAddressBook(): Promise<void> {
    ...
    console.log(
      `   Master Public Key: ${addressBook.masterPublicKey.substring(0, 20)}...`
    );
  ...
  private async createWalletFrontendAPI(): Promise<void> {
    ...
    try {
      ...
      console.log(
        `   Public Key: ${walletInfo.publicKey?.substring(0, 20)}...`
      );
  ...
  private printHeader(): void {
    ...
    console.log(
      "║" + " ".repeat(15) + "🗡️  SHOGUN WALLET 🗡️" + " ".repeat(15) + "║"
    );
    console.log(
      "║" +
        " ".repeat(12) +
...
async function main() {
  ...
  if (args.length < 2) {
    console.log(
      "Usage: yarn tsx ship/examples/wallet-cli.ts <username> <password>"
    );
    console.log("\nExample:");
